...
function getBlogPosts($mysqli, $in_title=null, $in_tag_id=null) {
  if (!empty($in_title)) {
    ...
    $command = 'SELECT blog_posts.* FROM blog_post_tags LEFT JOIN (blog_posts) ON (blog_post_tags.post_id = blog_posts.id) WHERE blog_post_tags.tag_id='.$tag_id.' ORDER BY blog_posts.date_posted DESC;';
  } else {
