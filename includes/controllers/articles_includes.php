...
function getBlogPosts($mysqli, $in_title=null, $in_tag_name=null) {
  if (!empty($in_title)) {
    ...
    $command = "SELECT blog_posts.* FROM blog_post_tags LEFT JOIN (blog_posts) ON (blog_post_tags.blog_post_id = blog_posts.id) WHERE blog_post_tags.tag_name='".$in_tag_name."';";
  } else {
    $command = "SELECT * FROM blog_posts ORDER BY date_posted DESC;";
  }
